...
class Row {
  ...
  get sortedGovernmentFields() {
    return this.governmentFields.slice().sort((f1, f2) => f1.label.localeCompare(f2.label));
  }
}
...
export default class GovernmentAreasPanel extends Component {
  ...
  *groupGovernmentFieldsByDomain() {
    let governmentAreas = yield this.args.governmentAreas;
    governmentAreas = governmentAreas?.slice();
    if (!governmentAreas) {
      return;
    }
    const domains = [];
