...
export default class SubcaseBekrachtigingDescriptionPanelView extends Component {
  ...
  loadAgendaData = task(async () => {
    ...
    const agendaActivities = await this.args.subcase
    ...
    for (const [index, agendaActivity] of sortedAgendaActivities
      ...
      await meeting?.kind;
      const decisionPublicationActivity = await meeting
        .belongsTo('internalDecisionPublicationActivity')
        .reload();
      await decisionPublicationActivity?.status; // used in get-functions above
      ...
      await decisionActivity?.belongsTo('decisionResultCode').reload();
