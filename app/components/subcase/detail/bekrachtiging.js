...
export default class SubcaseDetailBekrachtigingComponent extends Component {
  ...
  *ensureFreshData() {
  ...
  async setPreviousPiecesFromAgendaitem(deletedPiece) {
    ...
    if (documentContainer) {
      ...
      if (lastPiece) {
        const agendaActivities = await this.args.subcase.agendaActivities;
        const latestActivity = agendaActivities.sortBy('startDate')?.lastObject;
        if (latestActivity) {
          const agendaitems = await latestActivity
            .hasMany('agendaitems')
            .reload(); // This fixes a case where approving an agenda did not update latestAgendaitem
