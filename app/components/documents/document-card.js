...
export default class DocumentsDocumentCardComponent extends Component {
  ...
  *loadVersionHistory() {
  ...
  @task
  *saveUploadPiece(piece) {
    try {
      const previousPiece = this.sortedPieces.lastObject;
      let accessLevel = yield previousPiece.accessLevel;
      if (isBlank(accessLevel)) {
        accessLevel = yield this.store.findRecordByUri(
          'access-level',
          CONSTANTS.ACCESS_LEVELS.INTERN_REGERING
        );
      }
      ...
      this.pieces.removeObject(piece);
      yield this.fileService.deletePiece(piece);
      this.isOpenUploadModal = false;
      throw error;
    }
  }
  ...
  *deleteDocumentContainerWithUndo() {
  ...
  get newVersionName() {
    const previousPiece = this.sortedPieces.lastObject;
    return new VRDocumentName(previousPiece.name).withOtherVersionSuffix(
