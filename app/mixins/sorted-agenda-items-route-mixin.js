...
export default Mixin.create({
  ...
  async parseAgendaItems(agenda, session) {
    ...
    const agendaitemsToGroup = agendaitems.filter(
      item => !item.get("showAsRemark")
    );
    ...
    const groups = await this.agendaService.newSorting(
      session,
      agenda.get("id")
    );
    ...
    const minutesApproval = minutesApprovals
      .filter(item => item)
      .get("firstObject");
    return {
      groups,
  ...
  async model(params) {
    ...
    const {
      ...
      lastPrio,
      minutesApproval
    } = await this.parseAgendaItems(agenda, session, definite);
